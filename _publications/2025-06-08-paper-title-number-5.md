---
title: "C-Eval: A Multi-Level Multi-Discipline Chinese Evaluation Suite for Foundation Models"
collection: publications
category: conferences
permalink: /publication/ceval-multi-level-multi-discipline-chinese-evaluation-suite
excerpt: 'C-Eval is a multi-level, multi-discipline Chinese evaluation suite for foundation models.'
date: 2023-12-01
venue: 'NeurIPS 2023'
citation: 'Yuzhen Huang, Yuzhuo Bai, Zhihao Zhu, Junlei Zhang, Jinghan Zhang, Tangjun Su, Junteng Liu, Chuancheng Lv, Yikai Zhang, Jiayi Lei, Yao Fu, Maosong Sun, Junxian He. (2023). &quot;C-Eval: A Multi-Level Multi-Discipline Chinese Evaluation Suite for Foundation Models.&quot; <i>NeurIPS 2023</i>.'
---

C-Eval is a multi-level, multi-discipline Chinese evaluation suite for foundation models, covering four difficulty levels and dozens of disciplines, with both multiple-choice and free-response question sets.
